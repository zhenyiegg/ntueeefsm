...
function CircuitDiagram({
  ...
  useEffect(() => {
    ...
    const sketch = (p) => {
      ...
      const drawDiagram = () => {
        ...
        const nslHeight =
          numFlipFlops === "2" ? boxHeight + 200 : boxHeight + 340;
        ...
        for (let i = 0; i < numFlipFlops; i++) {
          ...
          p.line(
            flipFlopX + flipFlopWidth,
            flipFlopY + boxHeight / 2,
            flipFlopX + flipFlopWidth + 90,
            flipFlopY + boxHeight / 2,
          ); // Q Horizontal FF to middle vertical line
          p.line(
            flipFlopX + flipFlopWidth,
            bottomLabelY - 6,
            flipFlopX + flipFlopWidth + 90,
        ...
        if (fsmType === "Mealy") {
          ...
          if (numFlipFlops === "3") {
            ...
            p.line(
              ...
              outputY + boxHeight,
              outputX + boxWidth / 2,
              outputY + boxHeight + 270,
            ); // Connect to output logic
      ...
      p.mouseMoved = () => {
        ...
        if (
          x >= nextStateBox.x &&
          x <= nextStateBox.x + nextStateBox.w &&
          y >= nextStateBox.y &&
          y <= nextStateBox.y + nextStateBox.h
        ) {
          newHoveredBox = "nextState";
          if (p._curElement) p.cursor(p.HAND);
        } else if (
          x >= outputLogicBox.x &&
          x <= outputLogicBox.x + outputLogicBox.w &&
          y >= outputLogicBox.y &&
          y <= outputLogicBox.y + outputLogicBox.h
        ) {
  ...
  return (
    <div className="canvas-container">
      ...
      {/* Canvas container is always visible */}
    </div>
  );
}
