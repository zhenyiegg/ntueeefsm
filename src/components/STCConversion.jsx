...
import CircuitDiagram from "./circuitToState/CircuitDiagram"; // <-- Reuse your P5 circuit component
...
const STCConversion = ({
  ...
  useEffect(() => {
    ...
    for (let stateNum = 0; stateNum < Math.pow(2, maxBits); stateNum++) {
      ...
      for (const inputVal of possibleInputs) {
        ...
        console.log(`Unused State at: ${presentCode}, Input: ${inputVal}`);
    ...
    Object.keys(flipFlopInputs).forEach((ffKey) => {
      ...
      if (flipFlopType === "D" || flipFlopType === "T") {
        ...
        eqns[ffKey] = {
          ...
          canonicalSoM: getCanonicalSumOfMinterms(minterms, numVariables),
        ...
        table.forEach((entry) => {
          if (entry.J === 1) {
            const index = parseInt(entry.presentStateCode + entry.input, 2);
        ...
        eqns[`${ffKey}_J`] = {
          ...
          canonicalSoM: getCanonicalSumOfMinterms(mintermsJ, numVariables),
          canonicalPoM: getCanonicalProductOfMaxterms(
            mintermsJ,
            numVariables,
            validIndices,
        ...
        eqns[`${ffKey}_K`] = {
          ...
          canonicalSoM: getCanonicalSumOfMinterms(mintermsK, numVariables),
          canonicalPoM: getCanonicalProductOfMaxterms(
            mintermsK,
            numVariables,
            validIndices,
    ...
    if (diagramType === "Moore") {
      ...
      transitionTable.forEach((entry) => {
        if (entry.output === "1") {
          ...
          const presentStateCode = stateCodeMatch ? stateCodeMatch[1] : "";
    ...
    const numVarsForOutput =
      transitionTable.length > 0
        ? transitionTable[0].presentState.match(/\(([01]+)\)/)?.[1]?.length +
  ...
  const handleExcitationCellChange = (rowIndex, column, value) => {
    ...
    const capitalizedValue =
      column === "excitation" ? processedValue.toUpperCase() : processedValue;
      ...
      if (row.isUsed === false) {
        ...
        if (column === "input") {
          ...
          if (/^X+$/.test(userAnswer)) {
            ...
            if (/^[01X]+$/.test(userAnswer) && userAnswer.includes("X")) {
              isCorrect = true;
            }
        ...
        // Allow using X in place of a bit that doesn't matter
          ...
          for (let i = 0; i < userAnswer.length; i++) {
            if (userAnswer[i] !== "X" && userAnswer[i] !== correctValue[i]) {
  ...
  useEffect(() => {
    if (mergedExcitationTable) {
      ...
      switch (difficulty) {
        ...
        case "expert":
          ...
          mergedExcitationTable.forEach((_, rowIndex) => {
            ["input", "nextState", "excitation", "output"].forEach((column) => {
      ...
      mergedExcitationTable.forEach((_, rowIndex) => {
        ...
        const columns = ["input", "nextState", "excitation", "output"].sort(
          () => Math.random() - 0.5,
        );
  ...
  const renderExcitationCell = (row, rowIndex, column, value) => {
    ...
    const isIncorrect = excitationValidation.hasOwnProperty(key) && !isCorrect;
    ...
    switch (column) {
      ...
      case "excitation":
        ...
        maxLength = flipFlopType === "JK" ? numStateBits * 3 - 1 : numStateBits;
        break;
      default:
        maxLength = 1;
    }
    ...
    return (
      <div className="input-container">
        <input
          ...
          onChange={(e) =>
            handleExcitationCellChange(rowIndex, column, e.target.value)
          }
        ...
        {focusedExcitationCell === key && (
          <div className="input-tooltip">{excitationTooltip[key]}</div>
        )}
      </div>
    );
  };
      ...
      <div className="excitation-table-container">
        ...
        {showExcitationInfo && (
          <div className="info-tooltip">
            ...
            <p>This table shows the flip-flop input calculations:</p>
            <ul>
              <li>
                Present State: Binary encoding of current state ({numStateBits}{" "}
                bits)
              </li>
              <li>
                Next State: Binary encoding of next state ({numStateBits} bits)
              </li>
              {flipFlopType === "JK" ? (
                <li>
                  JK Inputs: Format is "J1K1 J0K0..." for {numStateBits} state
                  bits. Each JK pair can be 0X, X0, 1X, X1, 00, 01, 10, 11, or
                  XX. A value of "X" means "don't care" - either 0 or 1 can be
                  used.
                </li>
              ) : (
                <li>{flipFlopType} Inputs: Binary values for each flip-flop</li>
              )}
            </ul>
          ...
          <tbody>
            {mergedExcitationTable.map((row, idx) => (
              <tr key={idx}>
                ...
                <td>{renderExcitationCell(row, idx, "input", row.input)}</td>
                <td>
                  {renderExcitationCell(row, idx, "nextState", row.nextState)}
                </td>
                <td>
                  {renderExcitationCell(row, idx, "excitation", row.excitation)}
                </td>
                <td>{renderExcitationCell(row, idx, "output", row.output)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  const renderEquationInput = (key, field, variableName, equation) => {
    ...
    const inputVars =
      diagramType === "Moore" && key === "Z"
        ...
        : numInputs > 1
          ? Array.from({ length: numInputs }, (_, i) => `X${numInputs - 1 - i}`)
          : ["X0"];
        ...
        <span className="equation-value">
          ...
          <div className="input-container">
            <input
              ...
              onChange={(e) =>
                handleEquationCellChange(key, field, e.target.value)
              }
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.checked) {
                        ...
                        const hasNoTerms =
                          field === "minterms"
                            ? eqn?.mintermIndices.length === 0
                            : eqn?.maxtermIndices?.length === 0;
      ...
      <div className="equations-container">
        ...
        {showEquationsInfo && (
          <div className="info-tooltip">
            ...
            <ul>
              <li>
                Use the Sum of Minterms expression to identify the indices
              </li>
              <li>
                Separate multiple indices with commas (e.g., "0, 1, 4, 5")
              </li>
              <li>The indices should match the terms in the canonical sum</li>
              <li>
                The variables are listed in order (e.g., D0(Q1, Q0, X)) to
                ensure correct interpretation of minterms/maxterms
              </li>
            </ul>
            <p>
              Example: If sum of minterms is "x̄ȳz + x̄yz̄", the indices would be
              "1, 2"
            </p>
          </div>
        )}
                ...
                <div key={key} className="equation-block">
                  <h4>
                    {key === "Z"
                      ...
                      : key.includes("_")
                        ? flipFlopType === "JK"
                          ? `${key.includes("_J") ? "J" : "K"}${getFlipFlopName(
                              key.match(/\d+/)[0],
                              flipFlopType,
                            )} Equation`
                          : `${getFlipFlopName(
                  ...
                  <div className="equation-forms">
                    {renderEquationInput(key, "minterms", variableName, eqn)}
                    {renderEquationInput(key, "maxterms", variableName, eqn)}
                    {renderEquationInput(key, "sop", variableName, eqn)}
                  </div>
                </div>
              );
            })}
      ...
      Object.keys(simplifiedEquations).forEach((key) => {
        ...
        if (!equationValidation[maxtermKey]) {
          if (hasNoMaxterms) {
            ...
            newAnswers[maxtermKey] = eqn.canonicalPoM.match(/\((.*?)\)/)[1];
          }
          // Don't set validation to true for given-up fields
        }
  ...
  const calculateScore = () => {
    ...
    if (simplifiedEquations) {
      ...
      const numEquationFields = Object.keys(simplifiedEquations).length * 3;
      ...
      <div className="score-container">
        <div className="score-header">
          ...
          <div className="button-container">
            ...
            >
              <FontAwesomeIcon icon={faDownload} /> Convert to Netlist
            </button>
            <button
              className="complete-download-button"
            ...
            >
              <FontAwesomeIcon icon={faDownload} /> Download Complete Project
            </button>
          </div>
        </div>
        <div className="score-details">
          <p>
            ...
            <span className="score-total">{total}</span> correct answers
          </p>
  ...
  const convertToNetlist = () => {
    if (!simplifiedEquations || Object.keys(simplifiedEquations).length === 0) {
      return;
    }
    ...
    Object.keys(simplifiedEquations).forEach((key) => {
      ...
      const productTerms = sopExpression.split("+").map((term) => term.trim());
      ...
      productTerms.forEach((term) => {
        ...
        while (i < term.length) {
          ...
          if ((term[i] === "Q" || term[i] === "X") && i + 1 < term.length) {
            ...
            if (isInverted) {
              ...
              if (!intermediateSignals.has(`${varName}'`)) {
                ...
                intermediateSignals.set(`${varName}'`, notOutput);
                signalLevels.set(notOutput, notGateLevel);
              }

              const notSignal = intermediateSignals.get(`${varName}'`);
  ...
  const downloadExcitationTableCSV = () => {
    ...
    const presentStateBits = bitIndices.map((i) => `Q${i}`).join("");
    const nextStateBits = bitIndices.map((i) => `Q${i}*`).join("");
    const inputBits = Array.from({ length: numInputs }, (_, i) => `X${i}`).join(
      "",
    );
  ...
  const downloadEquationsCSV = () => {
    ...
    Object.keys(simplifiedEquations)
      ...
      .forEach((key) => {
        ...
        if (key === "Z") {
          ...
          const ffIndex = key.match(/\d+/)[0];
          displayName = `${getFlipFlopName(ffIndex, flipFlopType)}(${allVars})`;
        }
        ...
        const isZeroMinterms = equationAnswers[isZeroMintermsKey] || false;
        const isZeroMaxterms = equationAnswers[isZeroMaxtermsKey] || false;

        let minterms = isZeroMinterms ? "0" : equationAnswers[mintermKey] || "";
        let maxterms = isZeroMaxterms ? "1" : equationAnswers[maxtermKey] || "";
    ...
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `${getStandardizedFilename("equation")}.csv`);
    document.body.appendChild(link);
  ...
  const downloadAllDataCSV = () => {
    ...
    transitionTable.forEach((row) => {
      const csvRow = [row.presentState, row.input, row.nextState, row.output]
        .map((value) => `"${value}"`)
        .join(",");
      csvContent += csvRow + "\n";
    });
