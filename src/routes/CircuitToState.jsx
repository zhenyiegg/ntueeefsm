...
} from "../components/booleanConverter";
import {
  convertSOPToNetlist,
  convertPOSToNetlist,
} from "../components/booleanToNetlist";
...
const CircuitToState = () => {
  ...
  const generateTablesFromCustomEquations = (userCustomAnswers) => {
    ...
    if (preFillOption && preFillOption.startsWith("random")) {
      const preFillPercent =
        preFillOption === "random75"
          ? 0.75
          : preFillOption === "random50"
            ? 0.5
            : preFillOption === "random25"
              ? 0.25
              : 0;
      ...
      const preFillCountExcitation = Math.floor(
        totalExcitationFields * preFillPercent,
      );
      ...
      const excitationIndices = shuffleArray(
        Array.from({ length: totalExcitationFields }, (_, i) => i),
      ).slice(0, preFillCountExcitation);
      const stateIndices = shuffleArray(
        Array.from({ length: totalStateFields }, (_, i) => i),
      ).slice(0, preFillCountState);
      ...
      const prefilledStateTransition = newStateTransitionTable.map(
        (row, rowIndex) => {
          const updatedRow = { ...row };
  ...
  const formatKeyForDisplay = (key, numInputs, numFlipFlops, fsmType) => {
    const flipFlops = Array.from(
      { length: parseInt(numFlipFlops) },
      (_, i) => `Q${numFlipFlops - i - 1}`,
    );
    const inputs = Array.from(
      { length: parseInt(numInputs) },
      (_, i) => `X${numInputs - i - 1}`,
    );
    ...
    if (column === "nextState") {
      ...
      if (isValidNextState) {
        setUserStateTransitionInputs((prevInputs) => {
          ...
          const isTooShort =
            value.length > 0 && value.length < parseInt(numFlipFlops);
  ...
  const downloadFullExercise = async () => {
    ...
    const canvasCircuit = await html2canvas(
    ...
    const stateDiagramElement = document.getElementById(
    ...
    <div className="container">
      ...
      <div className="dropdown-container">
        <select
          ...
          onChange={(e) => handleDropdownChange("numInputs", e.target.value)}
        >
        ...
        <select
          ...
          onChange={(e) => handleDropdownChange("flipFlopType", e.target.value)}
        >
        ...
        <select
          ...
          onChange={(e) => handleDropdownChange("numFlipFlops", e.target.value)}
        >
        ...
        <select
          ...
          onChange={(e) => handleDropdownChange("fsmType", e.target.value)}
        >
        ...
        <select
          ...
          onChange={(e) =>
            handleDropdownChange("preFillOption", e.target.value)
          }
        >
      ...
      {popupVisible && (
        <div
          className="popup-overlay-netlist"
          onClick={() => setPopupVisible(false)}
        >
          <div
            className="popup-box-netlist"
                  ...
                  {image ? (
                    ...
                    <pre
                      style={{
                        ...
                        fontFamily: "monospace", // monospace makes indents uniform
                      }}
                    >
                      {JSON.stringify(
                        netlistEquations.find((e) => e.label === label)
                          ?.netlist,
                        null,
                        2,
                      )}
        ...
        <div className="content-box">
          ...
          {showCustomEqnInfo && (
            <div className="customEqn-tooltip">
              ...
              <p>
                <b>Moore FSM:</b> Output Z depends only on the current state,
                not input X.
              </p>
              <ul>
                <li>
          ...
          <p className="customEqn-subtitle">
            Select minterms (Σm) or maxterms (<span className="maxterm">Π</span>
            M) and enter terms separated by commas.
          </p>
      ...
      {/* Popup */}
      {showPopup && (
        <div className="error-popup-overlay" tabIndex={0}>
          <div
      ...
      <div
        className={`content-box excitation-box ${showExcitationTable ? "active" : ""}`}
      >
        ...
        {showExcitationInfo && (
          <div className="info-tooltip-cts">
            <p>
              ...
              <strong>0</strong> or <strong>1</strong>.<br />
              <br />
                ...
                <table border="1">
                  ...
                  <tbody>
                    {userExcitationInputs.map((row, rowIndex) => (
                      <tr key={rowIndex}>
                        <td className="currentState-column">
                          {row.currentState}
                        </td>
      ...
      <div
        className={`content-box ${showStateTransitionTable ? "active" : ""}`}
      >
        <div className="content-header">
          <div className="left-header">
        ...
        {showStateTransitionTable && stateTransitionTable.length > 0 && (
          <div>
            <table border="1">
              <thead>
                <tr>
                  {generateStateTransitionTableHeaders().map(
                    (header, index) => (
                      <th key={index}>{header}</th>
                    ),
                  )}
      ...
      <div
        className={`content-box stateDiagram-box ${showStateDiagram ? "active" : ""}`}
      >
        <div className="content-header">
          <div className="left-header">
          ...
          {showStateDiagramInfo && (
            <div className="info-tooltip-cts">
              <p>
                Hover or click the transition arrows to view the state
                transition details.
              </p>
            </div>
          )}
